...
static char *activity_name[ACTIVITY_COUNT] = { "overhead", "startup", "jacobi_step", "local comm", "global comm"};
...
void show_activity(bool enable) {
    if (!enable)
	return;
    ...
    printf("    %8d ms    %5.1f %%    unknown\n", (int) ums, upct);
    printf("    %8d ms    %5.1f %%    elapsed\n", (int) (elapsed * 1000.0), 100.0);
}
